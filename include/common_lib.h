...
// USE_VIO
// 以图像为基本单元的测量值，一张图像对应IMU数据（图像也有可能是空的）
//! 注意，因为做了硬件同步，所以默认相机和雷达的时间几乎是严格对应的，也就是说每一帧lidar都有对应的图像
// 但是可能会差一点点，可能图像会在lidar的前面或者后面一丢丢
struct MeasureGroupCamera     
{
    double img_offset_time_;
    double image_time_;
    std::deque<sensor_msgs::Imu::ConstPtr> cam_with_imu_;
    cv::Mat img_;
    MeasureGroupCamera()
    {
        image_time_ = 0.0;
        img_offset_time_ = 0.0; // 与雷达时间的偏移
    };
};
...
struct MeasureGroup {
    ...
    // 在这个工程中，相机和雷达是经过硬件同步、同时触发的，所以图像时间戳理论上和雷达是很接近的
    MeasureGroupCamera meas_group_cam_;
};
